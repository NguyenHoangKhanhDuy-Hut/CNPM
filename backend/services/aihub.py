...
"""
    ...
    async def genvideo(self, request: GenVideoRequest) -> GenVideoResponse:
        ...
        try:
            ...
            files = None
            if request.image:
                image_file = await self._image_str_to_upload_file(
                    request.image, name_prefix="input_reference"
                )
                files = {"input_reference": image_file}

            # Use the underlying httpx client for custom endpoints
            if files:
                resp = await http.post(f"{base_url}/videos", data=create_params, files=files)
            else:
                resp = await http.post(f"{base_url}/videos", json=create_params)
            resp.raise_for_status()
            ...
            while status in ("in_progress", "queued"):
                logger.info(f"Video {video_id} progress: {video_data.get('progress', 0)}%")
                await asyncio.sleep(2)
                resp = await http.get(f"{base_url}/videos/{video_id}")
                resp.raise_for_status()
                video_data = resp.json()
                status = video_data.get("status")

            if status == "failed":
                error_msg = (video_data.get("error") or {}).get("message") or "Video generation failed"
                raise RuntimeError(error_msg)

            # Extract CDN URL
            cdn_url = video_data.get("url") or video_data.get("video_url") or self._extract_cdn_url(video_data)
            if not cdn_url:
            ...
            actual_duration = self._safe_int(video_data.get("seconds"), default=requested_seconds)
            ...
            return GenVideoResponse(
                ...
                revised_prompt=video_data.get("revised_prompt"),
            )
